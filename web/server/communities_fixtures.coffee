@Fixture.Community =
  ...
  set: ->
    if db.communities.find().count() is 0 and Meteor.settings.public.environment isnt "production"

      db.communities.insert community for community in @data()

  clear: ->
    db.communities.remove community for community in @data()
